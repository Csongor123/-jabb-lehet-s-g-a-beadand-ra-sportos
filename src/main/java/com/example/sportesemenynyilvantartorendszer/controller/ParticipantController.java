...
public class ParticipantController {
    ...
    @GetMapping("/{id}/event")
    public ResponseEntity<Event> getParticipantEvent(@PathVariable Long id) {
    ...
    public ResponseEntity<Map<String, String>> registerForEvent(
        ...
        if (eventService.isDeadlinePassed(eventId)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
        ...
        if (registered) {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Map.of("message", "Unknown error during registration"));
        }
    }
